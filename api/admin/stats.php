...
try {
    ...
    $boletas_mes   = $db->query("SELECT COUNT(*) FROM boletas
        WHERE DATE_FORMAT(created_at,'%Y-%m')='$mes_actual'")->fetchColumn();
    $boletas_ocr_mes = $db->query("SELECT COUNT(*) FROM boletas
        WHERE origen='ocr' AND DATE_FORMAT(created_at,'%Y-%m')='$mes_actual'")->fetchColumn();
    $boletas_manual_mes = $db->query("SELECT COUNT(*) FROM boletas
        WHERE origen='manual' AND DATE_FORMAT(created_at,'%Y-%m')='$mes_actual'")->fetchColumn();
    $total_ocr = $db->query("SELECT COUNT(*) FROM boletas WHERE origen='ocr'")->fetchColumn();

    // ── Boletas por día (últimos 14 días) ──
    $stmt = $db->query("SELECT DATE_FORMAT(created_at,'%d/%m') as dia, COUNT(*) as total
        FROM boletas
        WHERE created_at >= DATE_SUB(NOW(), INTERVAL 14 DAY)
        GROUP BY DATE(created_at)
        ORDER BY DATE(created_at) ASC
        LIMIT 14");
    $boletas_por_dia = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // ── Completadas ──
    $completadas_mes = $db->query("SELECT COUNT(*) FROM boletas
        WHERE estado='completada' AND DATE_FORMAT(created_at,'%Y-%m')='$mes_actual'")->fetchColumn();
    ...
    echo json_encode([
        ...
        'boletas_por_dia'   => $boletas_por_dia,
